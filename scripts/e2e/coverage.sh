...
if [[ -d "playwright-coverage" ]]; then
    ...
    if command_exists npx && [[ -f "package.json" ]]; then
        ...
        if ls playwright-coverage/*.json 1> /dev/null 2>&1; then
            ...
            log_info "See docs/reference/domain/testing-coverage.md for setup instructions."
        fi
    fi
else
    ...
    log_info ""
    log_info "See docs/reference/domain/testing-coverage.md for detailed setup instructions."
fi
