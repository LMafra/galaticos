  ...
  (testing "without filters"
    (let [request {:params {}}
          result (with-redefs [players-db/find-all (fn [_] [{:name "P1"}])]
                  (invoke handlers/list-players request))
  ...
  (testing "found"
    (let [id (str (ObjectId.))
          ...
          result (with-redefs [players-db/find-by-id (fn [x] (when (= x id) player))]
                  (invoke handlers/get-player request))
          body (parse-body result)]
      (is (= 200 (:status result)))
  ...
  (testing "found includes evolution"
    (let [id (str (ObjectId.))
          ...
          result (with-redefs [players-db/find-by-id (fn [x] (when (= x id) player))
                  ...
                  (invoke handlers/get-player-detail-bundle request))
          body (parse-body result)]
      (is (= 200 (:status result)))
  ...
  (testing "success"
    (let [request {:json-body {:name "New" :position "FW"}}
          ...
          result (with-redefs [players-db/create (fn [_] created)]
                  (invoke handlers/create-player request))
          body (parse-body result)]
  ...
  (testing "success"
    (let [id (str (ObjectId.))
          ...
          result (with-redefs [players-db/exists? (fn [x] (= x id))
                  ...
                  (invoke handlers/update-player request))]
      (is (= 200 (:status result)))))
  ...
  (testing "success"
    (let [id (str (ObjectId.))
          ...
          result (with-redefs [players-db/exists? (fn [x] (= x id))
                  ...
                  (invoke handlers/delete-player request))
          body (parse-body result)]
      (is (= 200 (:status result)))
