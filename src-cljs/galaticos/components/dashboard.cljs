...
(defn dashboard
  ...
  (let [selected-champ      (r/atom "all")
        ...
        reconcile-message   (r/atom nil)
        reconcile-success?  (r/atom nil)
        refresh-timer-id    (r/atom nil)
        reconcile!          (fn []
                              (reset! reconcile-message nil)
                              (reset! reconciling? true)
                              (api/reconcile-stats
                               (fn [data]
                                 (reset! reconciling? false)
                                 (reset! reconcile-success? true)
                                 (reset! reconcile-message
                                         (or (:message data)
                                             (str (:updated data)
                                                  " jogador(es) atualizado(s).")))
                                 (effects/ensure-dashboard! {:force? true}))
                               (fn [err _resp]
                                 (reset! reconciling? false)
                                 (reset! reconcile-success? false)
                                 (reset! reconcile-message (str "Erro: " err)))))]
    (r/create-class
     {:component-did-mount
      (fn [_]
          ...
          [:div {:class "space-y-6"}
           [:div {:class "flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"}
            ...
            [:div {:class "flex flex-col gap-2 sm:flex-row sm:items-center sm:flex-wrap"}
             ...
             (when @reconcile-message
               [common/alert
                @reconcile-message
                :variant (if @reconcile-success? :success :error)
                :class "sm:w-full"])]]

           (cond
             dashboard-error
             [common/error-message dashboard-error]
              ...
              [common/card
               [:div {:class "flex flex-wrap items-center justify-between gap-3"}
                ...
                [:div {:class "flex flex-wrap gap-2"}
                 [common/button "Gols"
                  #(reset! active-tab :goals)
                  :variant (if (= @active-tab :goals) :primary :outline)]
                 [common/button "Assistências"
                  #(reset! active-tab :assists)
                  :variant (if (= @active-tab :assists) :primary :outline)]
                 [common/button "Partidas"
                  #(reset! active-tab :matches)
                  :variant (if (= @active-tab :matches) :primary :outline)]
                 [common/button "Títulos"
                  #(reset! active-tab :titles)
                  :variant (if (= @active-tab :titles) :primary :outline)]]]
               (case @active-tab
                 :goals
                 (if (seq top-goals)
                   [common/table
