     ...
     [:div {:class "flex items-center gap-3"}
      ...
      [:h1 {:class "text-base font-semibold text-slate-900"}
       (case (nav-route current-route)
         ...
         "Galáticos")]]
     [:div {:class "flex items-center gap-3"}
      [:button {:class "inline-flex rounded-lg p-2 text-slate-600 hover:bg-slate-100"
...
(defn layout [current-route content]
  (let [theme (get-in @state/app-state [:ui :theme])
    ...
    [:div {:class (common/merge-classes "min-h-screen bg-slate-50 text-slate-900" (when (= theme "dark") "app-dark"))}
