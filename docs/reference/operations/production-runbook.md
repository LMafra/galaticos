...
- **After** loading the Excel file, the **source of truth in production** for match **results** is the **`matches`** collection. Seed-imported matches (`excel-seed` / `python-seed`) store the score only (`player-statistics` empty); **per-match** scorers/assists exist for matches created or edited in the app. Player career totals come from the **Base de dados** sheet (`players.aggregated-stats` table baseline). That field is **derived** (read cache) and must stay aligned with UI matches after [reconciliation](../analytics/reconciliation-runbook.md) (`POST /api/aggregations/reconcile`, admin).
